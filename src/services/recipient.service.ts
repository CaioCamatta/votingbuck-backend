import { DonationsByMonth, DonationsByParty, Recipient, TopDonators } from '@interfaces/recipient.interface';
...
class RecipientService {
  public async getOrgData(orgId: number): Promise<any> {
    ...
    const orgInfo: Recipient = await app.db.recipient.findUnique({
      where: {
        id: orgId,
      },
    });
    ...
    const [donationsByMonth, topDonators, donationsByParty]: [DonationsByMonth, TopDonators, DonationsByParty] = [
      ...
      // Top individual donators in an recipient
